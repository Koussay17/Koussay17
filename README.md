<!-- 
  MAIN CONTAINER - Centered content wrapper
  This div centers all content horizontally on the page
-->
<div align="center">

  <!-- 
    HEADER BANNER - Animated wave banner with title and subtitle
    - Type: waving creates an animated wave effect
    - Gradient colors from dark blue to light gray
    - Text: "M. Koussay" as main title
    - Description: "Aerospace Engineering • Fluid Mechanics • Theoretical Physics"
    - Animation: fadeIn for smooth appearance
  -->
  <img width="100%" src="https://capsule-render.vercel.app/api?type=waving&height=260&color=0:070814,25:120f2b,55:1a1f4d,80:2c3e67,100:bfc7d5&text=M.%20Koussay&fontSize=56&fontColor=e9edf7&fontAlignY=38&desc=Aerospace%20Engineering%20•%20Fluid%20Mechanics%20•%20Theoretical%20Physics&descAlignY=58&descAlign=50&animation=fadeIn" />

  <!-- 
    SKILLS BADGES SECTION - Visual tags representing areas of expertise
    Each badge shows a different engineering/physics discipline
    Colors are coordinated in dark blue/gray theme
  -->
  <p>
    <img src="https://img.shields.io/badge/Aerospace-0d1020?style=for-the-badge&color=7c87a6"/>
    <img src="https://img.shields.io/badge/Fluid%20Mechanics-0d1020?style=for-the-badge&color=5a6b9a"/>
    <img src="https://img.shields.io/badge/Propulsion-0d1020?style=for-the-badge&color=6778a5"/>
    <img src="https://img.shields.io/badge/Heat%20Transfer-0d1020?style=for-the-badge&color=909db8"/>
    <img src="https://img.shields.io/badge/Theoretical%20Physics-0d1020?style=for-the-badge&color=c0c8d8"/>
  </p>

  <!-- HORIZONTAL DIVIDER LINE - Visual separator -->
  <hr style="border:1px solid #2c3e67; width:90%;">

  <!-- 
    TWO-COLUMN PROFILE SECTION
    Left column: Personal profile description
    Right column: Technical identity in code-block format
  -->
  <table width="90%">
    <tr>
      <!-- LEFT COLUMN: Profile Description -->
      <td width="50%" align="left">
        
        <h2 style="color:#e9edf7;">✦ Profile</h2>
        
        <p style="color:#c7ced9; line-height:1.6;">
          Engineering student specialized in aerospace-oriented fluid mechanics,
          developing strong expertise in:
        </p>
        
        <!-- Unordered list of technical skills -->
        <ul style="color:#c7ced9;">
          <li>Computational Fluid Dynamics (CFD)</li>
          <li>Propulsion systems</li>
          <li>Thermal transfer</li>
          <li>Aerodynamics</li>
          <li>Experimental validation</li>
          <li>Theoretical physics reasoning</li>
        </ul>
        
        <p style="color:#c7ced9;">
          My objective is to understand the physical laws governing engineering systems,
          not just simulate them.
        </p>
      
      </td>
      
      <!-- RIGHT COLUMN: Scientific Identity (Code-style representation) -->
      <td width="50%" align="left">
        
        <h2 style="color:#e9edf7;">✦ Scientific Identity</h2>
        
        <!-- Preformatted text block - simulates a YAML/JSON configuration file -->
        <pre style="background:#0b0f1f; color:#c7ced9; padding:14px; border-radius:8px;">
identity:
  aerospace engineering
  fluid mechanics
  energetics
  theoretical physics

scientific_axes:
  CFD
  propulsion
  heat transfer
  aerodynamics
  simulation
  experimentation

mindset:
  analytical
  research-driven
  physically grounded
        </pre>
      
      </td>
    </tr>
  </table>

  <!-- HORIZONTAL DIVIDER LINE -->
  <hr style="border:1px solid #2c3e67; width:90%;">

  <!-- 
    CORE DOMAINS SECTION - Four key specializations with icons
    Each domain has: icon, title, and brief description
  -->
  <h2 style="color:#e9edf7;">✦ Core Domains</h2>
  
  <table width="90%">
    <tr>
      <!-- Domain 1: Propulsion -->
      <td align="center" width="25%">
        <img src="https://cdn-icons-png.flaticon.com/512/3212/3212608.png" width="50">
        <br>
        <b style="color:#e9edf7;">Propulsion</b>
        <br>
        <span style="color:#c7ced9;">Thrust • Stability • Performance</span>
      </td>
      
      <!-- Domain 2: CFD -->
      <td align="center" width="25%">
        <img src="https://cdn-icons-png.flaticon.com/512/4149/4149673.png" width="50">
        <br>
        <b style="color:#e9edf7;">CFD</b>
        <br>
        <span style="color:#c7ced9;">Flow simulation • Modeling</span>
      </td>
      
      <!-- Domain 3: Heat Transfer -->
      <td align="center" width="25%">
        <img src="https://cdn-icons-png.flaticon.com/512/869/869869.png" width="50">
        <br>
        <b style="color:#e9edf7;">Heat Transfer</b>
        <br>
        <span style="color:#c7ced9;">Thermal systems • Coupling</span>
      </td>
      
      <!-- Domain 4: Theoretical Physics -->
      <td align="center" width="25%">
        <img src="https://cdn-icons-png.flaticon.com/512/2949/2949043.png" width="50">
        <br>
        <b style="color:#e9edf7;">Theoretical Physics</b>
        <br>
        <span style="color:#c7ced9;">Mathematical modeling</span>
      </td>
    </tr>
  </table>

  <!-- HORIZONTAL DIVIDER LINE -->
  <hr style="border:1px solid #2c3e67; width:90%;">

  <!-- 
    TECHNICAL STACK SECTION - Software and programming tools
    Badges showing tools: STAR-CCM+, MATLAB, Python, NumPy, C, C++, SQL, CATIA, Fusion360
  -->
  <h2 style="color:#e9edf7;">✦ Technical Stack</h2>
  
  <p>
    <img src="https://img.shields.io/badge/STAR--CCM+-0b0f1f?style=for-the-badge&color=9ca8be"/>
    <img src="https://img.shields.io/badge/MATLAB-0b0f1f?style=for-the-badge&color=9ca8be"/>
    <img src="https://img.shields.io/badge/Python-0b0f1f?style=for-the-badge&color=9ca8be"/>
    <img src="https://img.shields.io/badge/NumPy-0b0f1f?style=for-the-badge&color=9ca8be"/>
    <img src="https://img.shields.io/badge/C-0b0f1f?style=for-the-badge&color=6778a5"/>
    <img src="https://img.shields.io/badge/C++-0b0f1f?style=for-the-badge&color=6778a5"/>
    <img src="https://img.shields.io/badge/SQL-0b0f1f?style=for-the-badge&color=6778a5"/>
    <img src="https://img.shields.io/badge/CATIA-0b0f1f?style=for-the-badge&color=6778a5"/>
    <img src="https://img.shields.io/badge/Fusion360-0b0f1f?style=for-the-badge&color=6778a5"/>
  </p>

  <!-- HORIZONTAL DIVIDER LINE -->
  <hr style="border:1px solid #2c3e67; width:90%;">

  <!-- 
    RESEARCH ATMOSPHERE SECTION - Three-column breakdown of research areas
    Columns: Aerodynamics, Propulsion, Thermal Sciences
    Each contains a bullet list of subtopics
  -->
  <h2 style="color:#e9edf7;">✦ Research Atmosphere</h2>
  
  <table width="90%">
    <tr>
      <!-- Column 1: Aerodynamics research topics -->
      <td width="33%">
        <b style="color:#e9edf7;">Aerodynamics</b>
        <ul style="color:#c7ced9;">
          <li>External flows</li>
          <li>Profiles and bodies</li>
          <li>Drag analysis</li>
          <li>Simulation vs experiment</li>
        </ul>
      </td>
      
      <!-- Column 2: Propulsion research topics -->
      <td width="33%">
        <b style="color:#e9edf7;">Propulsion</b>
        <ul style="color:#c7ced9;">
          <li>Rocket systems</li>
          <li>Sizing logic</li>
          <li>Ballistic stability</li>
          <li>Performance analysis</li>
        </ul>
      </td>
      
      <!-- Column 3: Thermal Sciences research topics -->
      <td width="33%">
        <b style="color:#e9edf7;">Thermal Sciences</b>
        <ul style="color:#c7ced9;">
          <li>Heat exchangers</li>
          <li>Thermal coefficients</li>
          <li>Inverse methods</li>
          <li>Prediction models</li>
        </ul>
      </td>
    </tr>
  </table>

  <!-- HORIZONTAL DIVIDER LINE -->
  <hr style="border:1px solid #2c3e67; width:90%;">

  <!-- 
    FUTURE OBSERVATORY SECTION - Placeholder for future content
    Three columns: Research Repositories, Scientific Papers, Laboratory Work
    Currently contains placeholder items (repo-name-01, etc.)
  -->
  <h2 style="color:#e9edf7;">✦ Future Observatory</h2>
  
  <table width="90%">
    <tr>
      <!-- Column 1: GitHub repositories placeholder -->
      <td width="33%">
        <h3 style="color:#e9edf7;">Research Repositories</h3>
        <ul style="color:#c7ced9;">
          <li>repo-name-01</li>
          <li>repo-name-02</li>
          <li>repo-name-03</li>
        </ul>
      </td>
      
      <!-- Column 2: Academic papers placeholder -->
      <td width="33%">
        <h3 style="color:#e9edf7;">Scientific Papers</h3>
        <ul style="color:#c7ced9;">
          <li>paper-title-01</li>
          <li>paper-title-02</li>
          <li>paper-title-03</li>
        </ul>
      </td>
      
      <!-- Column 3: Lab projects placeholder -->
      <td width="33%">
        <h3 style="color:#e9edf7;">Laboratory Work</h3>
        <ul style="color:#c7ced9;">
          <li>lab-project-01</li>
          <li>lab-project-02</li>
          <li>lab-project-03</li>
        </ul>
      </td>
    </tr>
  </table>

  <!-- HORIZONTAL DIVIDER LINE -->
  <hr style="border:1px solid #2c3e67; width:90%;">
  
  <!-- Duplicate divider (kept as in original) -->
  <hr style="border:1px solid #2c3e67; width:90%;">

  <!-- 
    GITHUB ACTIVITY & LANGUAGES SECTION
    Two-part layout: GitHub Stats (left) and Languages (right)
  -->
  <h2 align="center" style="color:#e9edf7;">
    ✦ GitHub Activity & Languages
  </h2>

  <div align="center">
    
    <!-- Table for GitHub Stats -->
    <table width="50%">
      <tr>
        <!-- LEFT: GitHub statistics card -->
        <td width="50%">
          <h3 style="color:#e9edf7; margin-bottom:10px;">
            GitHub Stats
          </h3>
          
          <!-- Styled container for the stats image -->
          <div style="display:inline-block; padding:16px; border:1px solid #4b5671; border-radius:10px; background-color:rgba(11,15,31,0.4);">
            <img height="170"
                 src="https://github-readme-stats.vercel.app/api?username=Koussay17&show_icons=true&theme=transparent&title_color=eceff7&text_color=b6bfd0&icon_color=8da2d1&border_color=00000000&bg_color=00000000">
          </div>
        </td>
      </tr>
    </table>

    <!-- Table for Language badges -->
    <table width="50%">
      <tr>
        <!-- RIGHT: Language proficiency badges -->
        <td width="50%" align="center">
          <h3 style="color:#e9edf7; margin-bottom:10px;">
            Most Used Languages
          </h3>
          
          <p>
            <img src="https://img.shields.io/badge/French-Native-0b0f1f?style=for-the-badge&logo=france&logoColor=white&color=bfc7d5"/>
            <br>
            <img src="https://img.shields.io/badge/English-Professional-0b0f1f?style=for-the-badge&logo=unitedkingdom&logoColor=white&color=9ca8be"/>
          </p>
        </td>
      </tr>
    </table>

  </div>

  <!-- HORIZONTAL DIVIDER LINE -->
  <hr style="border:1px solid #2c3e67; width:90%;">
  
  <!-- 
    CONTACT LINKS SECTION
    Email and LinkedIn profile links as clickable badges
  -->
  <p>
    <a href="mailto:koussay_mansouri@yahoo.com">
      <img src="https://img.shields.io/badge/Email-0d1020?style=for-the-badge&color=909db8">
    </a>
    
    <a href="https://linkedin.com/in/koussay-mansouri">
      <img src="https://img.shields.io/badge/LinkedIn-0d1020?style=for-the-badge&color=909db8">
    </a>
  </p>

  <br>

  <!-- 
    FOOTER BANNER - Waving banner at bottom of page
    Similar to header but with section=footer parameter
    Creates a cohesive visual frame for the profile
  -->
  <img width="100%" src="https://capsule-render.vercel.app/api?type=waving&height=160&section=footer&color=0:05070b,25:120f2b,55:1a1f4d,80:2c3e67,100:bfc7d5">

</div>
